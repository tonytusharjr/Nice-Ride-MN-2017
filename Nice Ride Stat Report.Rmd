...
#Plot data
gg_prop_Account %+% Trips_Account16 
gg_prop_Account %+% Trips_Account17 
multiplot(gg_prop_DayEnd %+% Trips_DayEnd16, gg_prop_DayEnd %+% Trips_DayEnd17)
```
...
#Visualize with outliers and without outliers via boxplots
```{r}
#Setup plot properties for possible reuse
gg_prop_boxplot <- ggplot(data = data.frame(), aes(Start_Month, Total_DurationMin, color=factor(Start_Year))) + geom_boxplot(outlier.shape = NA) + labs(x = "", y = "", title="", subtitle="", color="") + guides(fill=FALSE) + scale_x_continuous(limits = c(6.80, 24.50))

#Plot data
gg_prop_boxplot %+% Rides1617 
gg_prop_boxplot %+% Rides1617_Mod 
multiplot(gg_prop_DayEnd %+% Trips_DayEnd16, gg_prop_DayEnd %+% Trips_DayEnd17)
```
